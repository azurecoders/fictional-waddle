...
// Create a type for the form values that matches LanguagesType without the id
...
const LanguageForm: React.FC<LanguageFormProps> = ({
  ...
  const handleSubmit = (
    ...
    setLanguages((current) => [
      ...current,
      {
        id: Math.floor(Math.random() * 10000), // Better random ID generation
        ...values,
      },
    ]);
    setSubmitting(false);
        ...
        >
          {({ values, errors, touched, isSubmitting }) => (
            <Form className="max-w-3xl mx-auto space-y-8">
              <div className="bg-neutral-900/70 backdrop-blur-xl rounded-xl p-8 shadow-lg border border-neutral-800">
                ...
                {({ push: pushSection, remove: removeSection }) => (
                  <>
                    {values.sections.map((section, sectionIndex) => (
                      <div
                        key={section.id}
                        className="bg-neutral-900/70 backdrop-blur-xl rounded-xl p-8 shadow-lg 
